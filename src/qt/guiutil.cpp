...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a lpscoins address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no lpscoins: URI
...
{
    // Convert lpscoins:// to lpscoins:
    //
    //    Cannot handle this later, because lpscoins:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open lpscoins.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "lpscoins.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for lpscoins.lnk
...
{
    return GetAutostartDir() / "lpscoins.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a lpscoins.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=lpscoins\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lpscoins app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add lpscoins app to startup item list
