...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizelpscoinsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizelpscoinsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizelpscoinsAmount = strAnonymizelpscoinsAmount.remove(strAnonymizelpscoinsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizelpscoinsAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizelpscoinsAmount * COIN) nMaxToAnonymize = nAnonymizelpscoinsAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizelpscoinsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizelpscoinsAmount));
        strAnonymizelpscoinsAmount = strAnonymizelpscoinsAmount.remove(strAnonymizelpscoinsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizelpscoinsAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
